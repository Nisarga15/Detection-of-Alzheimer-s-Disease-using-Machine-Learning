...
app.config["IMAGE_UPLOADS"] = r"C:\Users\HP\Desktop\Updated Code Alzheimer’s Disease/web_app_with_ml_backend/input_imgs"
app.config["ALLOWED_IMAGE_EXTENSIONS"] = ["JPEG", "JPG", "PNG"]
app.config["IMAGE_HEATMAP"] = r"C:\Users\HP\Desktop\Updated Code Alzheimer’s Disease/web_app_with_ml_backend/heat_map"
...
def login():
    ...
    #con=sqlite3.connect('C:/Users/HP/Desktop/Updated Code Alzheimer’s Disease/web_app_with_ml_backend/login1.db')
    con=sqlite3.connect(db_path)
    cursor=con.cursor()
    #query="CREATE TABLE 'login1' ( '{}' TEXT, '{}' TEXT )".format(name, email)
    #cursor.execute(query)
    query="INSERT INTO 'login' VALUES ('{}', '{}')".format(name, email)
    cursor.execute(query)
    con.commit()
    #cursor.close()
    return render_template('index.html')
...
def result():
    try:
        if request.method=="GET":
            ...
            con=sqlite3.connect('C:/Users/HP/Desktop/Updated Code Alzheimer’s Disease/web_app_with_ml_backend/login.db')
            cursor=con.cursor()
            query="SELECT name FROM 'login' WHERE email=('{}')".format(email)
            res=cursor.execute(query)
            res=res.fetchall()[0][0]
            return render_template('appointment.html',name=res)
    except:
        return render_template('appointment.html',name=" ")
